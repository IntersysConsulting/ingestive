...
public class ErrorLogBolt implements IRichBolt, IErrorLogBolt {

	private static final long serialVersionUID = 1L;
	private OutputCollector collector;
	private Map<String, Object> config;
    private static final IIngestionLogger log = IngestionTopology.getLogger();
	private ErrorLogBoltLogic logic = new ErrorLogBoltLogic();
    ...
    public void execute(Tuple tuple) {
    	anchor = tuple;
	    try {
	    	if ("Errors".equals(tuple.getSourceStreamId())) {
	    		ParseError parseError = new ParseError(tuple);
	            logic.readFromErrors(parseError, this);
	    	}
		} catch (Exception e) { 
			log.severe("ErrorLogBolt", "execute", "Error executing Tuple", e); 
	    }
	}
