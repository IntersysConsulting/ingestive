...
public class EventProcessorBolt implements IRichBolt, IEventProcessorBolt {

	private static final long serialVersionUID = 1L;
	private OutputCollector collector;
	private Map<String, Object> config;
    private static final IIngestionLogger log = IngestionTopology.getLogger();
	private EventProcessorBoltLogic logic = new EventProcessorBoltLogic();
    ...
    public void execute(Tuple tuple) {
    	anchor = tuple;
	    try {
	    	if ("Events".equals(tuple.getSourceStreamId())) {
	    		Event event = new Event(tuple);
	            logic.readFromEvents(event, this);
	    	}
		} catch (Exception e) { 
			log.severe("EventProcessorBolt", "execute", "Error executing Tuple", e); 
	    }
	}
