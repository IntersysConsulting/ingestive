...
public class NewAccountProcessorBolt implements IRichBolt, INewAccountProcessorBolt {

	private static final long serialVersionUID = 1L;
	private OutputCollector collector;
	private Map<String, Object> config;
    private static final IIngestionLogger log = IngestionTopology.getLogger();
	private NewAccountProcessorBoltLogic logic = new NewAccountProcessorBoltLogic();
    ...
    public void execute(Tuple tuple) {
    	anchor = tuple;
	    try {
	    	if ("NewAccounts".equals(tuple.getSourceStreamId())) {
	    		Account account = new Account(tuple);
	            logic.readFromNewAccounts(account, this);
	    	}
		} catch (Exception e) { 
			log.severe("NewAccountProcessorBolt", "execute", "Error executing Tuple", e); 
	    }
	}
